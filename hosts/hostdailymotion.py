...
config.plugins.iptvplayer.dailymotion_localization = ConfigSelection(default='auto', choices=[('auto', _('auto')),
 ('ar_AA', '\xd8\xa7\xd9\x84\xd8\xb9\xd8\xb1\xd8\xa8\xd9\x8a\xd8\xa9'),
 ('es_AR', 'Argentina'),
 ('en_AU', 'Australia'),
 ('de_AT', '\xc3\x96sterreich'),
 ('nl_BE', 'Belgi\xc3\xab'),
 ('fr_BE', 'Belgique'),
 ('pt_BR', 'Brasil'),
 ('en_CA', 'Canada'),
 ('fr_CA', 'Canada'),
 ('zh_CN', '\xe4\xb8\xad\xe5\x9b\xbd'),
 ('fr_FR', 'France'),
 ('de_DE', 'Deutschland'),
 ('el_GR', '\xce\x95\xce\xbb\xce\xbb\xce\xac\xce\xb4\xce\xb1'),
 ('en_IN', 'India'),
 ('id_ID', 'Indonesia'),
 ('en_EN', 'International'),
 ('en_IE', 'Ireland'),
 ('it_IT', 'Italia'),
 ('ja_JP', '\xe6\x97\xa5\xe6\x9c\xac'),
 ('ms_MY', 'Malaysia'),
 ('es_MX', 'M\xc3\xa9xico'),
 ('fr_MA', 'Maroc'),
 ('nl_NL', 'Nederland'),
 ...
 ('pl_PL', 'Polska'),
 ('pt_PT', 'Portugal'),
 ('ro_RO', 'Rom\xc3\xa2nia'),
 ('ru_RU', '\xd0\xa0\xd0\xbe\xd1\x81\xd1\x81\xd0\xb8\xd1\x8f'),
 ('en_SG', 'Singapore'),
 ('ko_KR', '\xeb\x8c\x80\xed\x95\x9c\xeb\xaf\xbc\xea\xb5\xad'),
 ('es_ES', 'Espa\xc3\xb1a'),
 ('fr_CH', 'Suisse'),
 ('it_CH', 'Svizzera'),
 ('de_CH', 'Schweiz'),
 ('fr_TN', 'Tunisie'),
 ('tr_TR', 'T\xc3\xbcrkiye'),
 ('en_GB', 'United Kingdom'),
 ('en_US', 'United States'),
 ('vi_VN', 'Vi\xe1\xbb\x87t Nam')])
...
class Dailymotion(CBaseHostClass):
    ...
    def __init__(self):
        CBaseHostClass.__init__(self, {'history': 'Dailymotion',
         'cookie': 'dailymotion.cookie'})
        ...
        self.MAIN_CAT_TAB = [{'category': 'categories',
          'title': _('Categories')}, {'category': 'search',
          'title': _('Search'),
          'search_item': True}, {'category': 'search_history',
          'title': _('Search history')}]
        self.SORT_TAB = [{'title': _('Most viewed'),
          'sort': 'visited'},
         {'title': _('Most recent'),
          'sort': 'recent'},
         {'title': _('Most rated'),
    ...
    def getLocale(self):
        ...
        if 'auto' != locale:
            return locale
        all = [('ar_AA', '\xd8\xa7\xd9\x84\xd8\xb9\xd8\xb1\xd8\xa8\xd9\x8a\xd8\xa9'),
         ('es_AR', 'Argentina'),
         ('en_AU', 'Australia'),
         ('de_AT', '\xc3\x96sterreich'),
         ('nl_BE', 'Belgi\xc3\xab'),
        ...
        try:
            ...
            for item in data['list']:
                ...
                if item[mode_key] == 'vod':
                    ...
                    params.update({'title': item[title_key],
                     'url': item[url_key],
                     'icon': item.get(icon_key, ''),
                     'desc': desc})
    ...
    def getAuthToken(self):
        if '' in (self.authData['client_id'],
         self.authData['client_secret'],
         self.authData['visitor_id'],
         self.authData['traffic_segment'],
         self.authData['url'],
         self.authData['auth_url'],
         self.authData['token']):
            rm(self.COOKIE_FILE)
        ...
        if self.authData.get('expires', 0) < int(time.time()):
            ...
            post_data = {'client_id': self.authData['client_id'],
             'client_secret': self.authData['client_secret'],
             'grant_type': self.authData['grant_type'],
             'visitor_id': self.authData['visitor_id'],
             'traffic_segment': self.authData['traffic_segment']}
    ...
    def getApiHeaders(self, cItem):
        params = {}
        params['header'] = {'User-Agent': self.HTTP_HEADER['User-Agent'],
    ...
    def listSiteSeach(self, cItem):
        ...
        post_data = post_data % (cItem['f_query'],
         pages.get('videos', 1),
         pages.get('lives', 1),
         pages.get('channels', 1),
         pages.get('playlists', 1),
         limits.get('videos', 0),
         limits.get('lives', 0),
         limits.get('channels', 0),
         limits.get('playlists', 0),
         urllib.quote(cItem['f_query']),
         cItem['f_type'])
            ...
            for item in data['edges']:
                ...
                if item['__typename'] == 'Collection':
                    ...
                    params = {'good_for_fav': True,
                     'name': 'category',
                     'category': 'list_playlist',
                     'title': title,
                     'f_xid': item['xid'],
                     'icon': item['thumbURLx480'],
                     'desc': '[/br]'.join(desc)}
                    self.addDir(params)
