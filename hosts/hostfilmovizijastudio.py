...
class FilmovizijaStudio(CBaseHostClass):
    ...
    def __init__(self):
        ...
        my = 'years.php'
        self.MAIN_CAT_TAB = [{'category': 'list_movie_cats',
          'title': _('Movies'),
          'url': self.getFullUrl(mc),
          'filter': 'movies'},
         {'category': 'categories',
          'title': _('New Movies'),
          'url': self.getMainUrl(),
    ...
    def getPage2(self, baseUrl, params = {}, post_data = None):
        params['cloudflare_params'] = {'domain': 'www.filmovizija.tv',
            ...
            try:
                ...
                if urlClass == 'direct':
                    if not urlId.startswith('http'):
                        urlTab.append({'name': urlName,
                         'url': 'id=' + urlId,
                         'need_resolve': 1})
                    else:
                        urlTab.append({'name': urlName,
                         'url': self._getFullUrl(urlId),
                         'need_resolve': 1})
            ...
            for item in subData:
                if 'captions' in item:
                    ...
                    if not src.startswith('http'):
                        continue
                    sub_tracks.append({'title': label,
                     'url': self._getFullUrl(src),
                     'lang': label,
                     'format': 'srt'})
