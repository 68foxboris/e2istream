...
def gettytul():
    return 'https://thewatchcartoononline.tv/'


class WatchCartoonOnline(CBaseHostClass):

    def __init__(self):
        CBaseHostClass.__init__(self, {'history': 'watchcartoononline.com',
         'cookie': 'watchcartoononline.com.cookie'})
    ...
    def listMainMenu(self, cItem):
        ...
        def _fillItems(data):
            ...
            if len(tabItems):
                params = dict(cItem)
                params.update({'category': 'sub_items',
                 'title': tabTitle,
                 'sub_items': tabItems})
                self.addDir(params)
    ...
    def listABC(self, cItem, nextCategory):
        ...
        for idx in range(len(data)):
            ...
            if len(tabItems):
                params = dict(cItem)
                params.update({'category': 'sub_items',
                 'title': tabTitle + ' ({0}) '.format(len(tabItems)),
                 'sub_items': tabItems})
                self.addDir(params)
    ...
    def listSearchResult(self, cItem, searchPattern, searchType):
        ...
        self.setMainUrl(self.cm.meta['url'])
        url = self.getFullUrl('/search')
        sts, data = self.getPage(url, {}, {'catara': searchPattern,
         'konuara': searchType})
        if not sts:
            return
    ...
    def getLinksForVideo(self, cItem):
        ...
        if playerUrl != '':
            urlTab.append({'name': self.up.getDomain(self.cm.meta['url']).replace('www.', ''),
             'url': self.cm.getFullUrl(playerUrl, self.cm.getBaseUrl(cUrl)),
             'need_resolve': 1})
        ...
        playerUrl = self._getPlayerData(data)
        if playerUrl != '':
            urlTab.append({'cache_key': cacheKey,
             'name': self.up.getDomain(self.cm.meta['url']).replace('www.', ''),
             'url': self.cm.getFullUrl(playerUrl, self.cm.getBaseUrl(cUrl)),
             'need_resolve': 1})
        if len(urlTab):
    ...
    def getVideoLinks(self, videoUrl):
        ...
        for item in items:
            ...
            if url not in uniqueUrls:
                ...
                if not url:
                    continue
                urlTab.append({'name': '[{0}] {1} {2}'.format(type.split('/', 1)[0], label, domain),
        ...
        if not urlTab:
            ...
            url = self.cm.getFullUrl(ph.search(data, '[\'"]([^\'^"]*?getvidlink\\.php[^\'^"]*?)[\'"]')[0], cUrl)
            sts, data = self.getPage(url, params)
            if not sts:
                return
            try:
                data = json_loads(data)
                url = data['server'] + '/getvid?evid=' + data['enc']
                urlTab.append({'name': '',
