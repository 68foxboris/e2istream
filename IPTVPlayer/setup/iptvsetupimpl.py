...
class IPTVSetupImpl:
    ...
    def rtmpdumpStep(self, ret=None):
        ...
        self.stepHelper.updateMessage('detection', (_('The "%s" utility is used by the %s to buffering and downloading [%s] links.') % ('rtmpdump', 'E2iStream', 'rtmp, rtmpt, rtmpe, rtmpte, rtmps')), 1)
    ...
    def uchardetStep(self, ret=None):
        ...
        self.stepHelper.updateMessage('detection', _('The "%s" utility is used by the %s to determine the encoding of the text.') % ('E2iStream', 'uchardet'), 1)
    ...
    def binaryDetectFinished(self, stsTab, dataTab):
        ...
        if len(stsTab) > 0 and True == stsTab[-1]:
            ...
            if self.stepHelper.isDebugMessagesAllowed():
                self.showMessage(_("[%s] will be used by %s.") % (path, 'E2iStream'), MessageBox.TYPE_INFO, self.finish)
            else:
