...
class IPTVUpdateWindow(Screen):
    ...
    def __init__(self, session, updateObjImpl, autoStart=True):
        printDBG("..:: E2iStream ::..   IPTVUpdateWindow(Screen): __init__(self, session, updateObjImpl, autoStart=True):")
    ...
    def __onClose(self):
        printDBG("..:: E2iStream ::..   IPTVUpdateWindow(Screen): __onClose(self):")
    ...
    def onStart(self):
        printDBG("..:: E2iStream ::..   IPTVUpdateWindow(Screen): onStart(self):")
    ...
    def doStart(self):
        printDBG("..:: E2iStream ::..   IPTVUpdateWindow(Screen): doStart(self):")
        if 0 < len(self.list):
            self.currStep = 0
            self.status     = 'working'
            self.stepExecute()
    ...
    def reloadList(self):
        printDBG("..:: E2iStream ::..   IPTVUpdateWindow(Screen): reloadList(self):")
        self["list"].hide()
        self["list"].setList([ (x,) for x in self.list])
        self["list"].show()
      
    def keyOK(self):
        printDBG("..:: E2iStream ::..   IPTVUpdateWindow(Screen): keyOK(self):")
    ...
    def keyExit(self):
        printDBG("..:: E2iStream ::..   IPTVUpdateWindow(Screen): keyExit(self):")
    ...
    def preparUpdateStepsList(self):
        printDBG("..:: E2iStream ::..   IPTVUpdateWindow(Screen): preparUpdateStepsList(self):")
        self.list = self.updateObjImpl.getStepsList()
        self.reloadList()
        
    def stepExecute(self):
        printDBG("..:: E2iStream ::..   IPTVUpdateWindow(Screen): stepExecute(self):")
    ...
    def stepFinished(self, stsCode, msg):
        printDBG("..:: E2iStream ::..   IPTVUpdateWindow(Screen): stsCode[%d], msg[%s]" % (stsCode, msg))
        nextStep = True
...
class UpdateMainAppImpl(IUpdateObjectInterface):
    ...
    def __init__(self, session, allowTheSameVersion=False):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): __init__(self, session, allowTheSameVersion=False):")
        self.SERVERS_LIST_URLS = [GetUpdateServerUri('versionlist.json')]
        ...
        self.setup_title = _("E2iStream - update")
...
#        self.gitlabList  = {}
        ...
        self.messages['problem_removal'] = _("Problem removing the previous version.\nStatus[%d], outData[%s].")
        self.messages['problem_install'] = _("Problem installing the new version.\nStatus[%d], outData[%s]")
        self.messages['problem_copy']    = _("Problem copying files.\nStatus[%d], outData[%s]")
    ...
    def checkVersionFile(self, newVerPath):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): checkVersionFile(self, newVerPath): newVerPath = [%s]" %newVerPath)
        code = 0
        ...
        if os_path.isfile(newVerFile):
            ...
            printDBG("..:: E2iStream ::..   version.py verPattern = [%s]" %verPattern)
        ...
        try:
            ...
            if newVerNum != self.serversList[self.currServIdx]['version']:
                code = -1
                msg  = _("Wrong version. \n downloaded version [%s] is different from [%s].") % (newVerNum, self.serversList[self.currServIdx]['version'])
        except Exception:
            printExc()
            code = -1
    ...
    def doRestart(self, *args):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): doRestart(self, *args):")
        try:
            try:
                ...
                printExc()
                msgtxt = _("Please Note: You use this plugin at your own risk.")
                self.session.open(MessageBox, _("Restarting after E2iStream updated to version[%s].\n\n") % self.serversList[self.currServIdx]['version'] + msgtxt, type = MessageBox.TYPE_INFO, timeout = 5 )
    ...
    def doRestartFailed(self, *args):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): doRestartFailed(self, *args):")
        try:
            self.list[currStep].update( {'info': _("Aborted"), 'icon': self.ICON.CANCELLED} )
        except Exception:
            printExc()
        self.session.open(MessageBox, _("Failed to restart. \nPlease restart manually."), type = MessageBox.TYPE_INFO, timeout=5 )

    #########################################################
    # INREFACE IMPLEMENTATION METHODS
    #########################################################
    def getSetupTitle(self):
        return _("E2iStream - update")
    ...
    def finalize(self, success=True, errorMsg=""):
        if success:
            self.session.openWithCallback(self.doRestart, MessageBox, _("Update completed successfully, your system will reboot."), type = MessageBox.TYPE_INFO, timeout=10 )
            return False
        else:
            message = errorMsg 
            # Failed message:
            message += "\n\n" + _("Update failed.\nIts possible you may have the latest version.") 
    ...
    def getStepsList(self):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): getStepsList(self):")
        self.list = []
              
        self.list.append( self.__getStepDesc(title = _("Reading available versions."),       execFunction = self.stepGetServerLists ) )
        self.list.append( self.__getStepDesc(title = _("Downloading update packet."),       execFunction = self.stepGetArchive ) )
        self.list.append( self.__getStepDesc(title = _("Extracting update packet."),        execFunction = self.stepUnpackArchive ) )
        self.list.append( self.__getStepDesc(title = _("Copying installed binaries."),      execFunction = self.stepCopyPostInatalledBinaries, breakable=True, ignoreError=True ) )
        self.list.append( self.__getStepDesc(title = _("Executing custom scripts."),        execFunction = self.stepExecuteUserScripts ) )
        self.list.append( self.__getStepDesc(title = _("Checking new version files."),      execFunction = self.stepCheckFiles ) )
    ...
    def isReadyToExecuteStep(self, currStepIdx):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): isReadyToExecuteStep(self, currStepIdx):")
    ...
    def stepGetServerLists(self):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): stepGetServerLists(self):")
        self.clearTmpData()
    ...
    def stepGetArchive(self):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): stepGetArchive(self):")
    ...
    def stepUnpackArchive(self):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): stepUnpackArchive(self):")
    ...
    def stepGetEncKey(self):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): stepGetEncKey(self):")
        from hashlib import sha256
    ...
    def stepDecryptArchive(self):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): stepDecryptArchive(self): sourceArchive[%s] " % (self.sourceArchive))
        from hashlib import sha256
        try:
    ...
    def stepGetGraphicsArchive(self):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): stepGetGraphicsArchive(self):")
    ...
    def stepUnpackGraphicsArchive(self):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): stepUnpackGraphicsArchive(self):")
        if '' == self.serversList[self.currServIdx]['graphics_url']:
            self.stepFinished(0, _('Skipped.'))
            return
    ...
    def stepGetIconsArchive(self):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): stepGetIconsArchive(self):")
    ...
    def stepUnpackIconsArchive(self):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): stepUnpackIconsArchive(self):")
        if '' == self.serversList[self.currServIdx]['icons_url'] or \
            not config.plugins.iptvplayer.ListaGraficzna.value:
            self.stepFinished(0, _('Skipped.'))
            return
    ...
    def stepCheckFiles(self):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): stepCheckFiles(self):")
    ...
    def stepRemoveUnnecessaryFiles(self):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): stepRemoveUnnecessaryFiles(self):")
    ...
    def stepCopyGraphicsWithoutIcons(self):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): stepCopyGraphicsWithoutIcons(self):")
    ...
    def stepCopyAllGraphics(self):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): stepCopyAllGraphics(self):")
        # copy whole old icon directory
        oldIconsDir = os_path.join(self.ExtensionPath, 'IPTVPlayer', 'icons')
        newIconsDir = os_path.join(self.ExtensionTmpPath, 'IPTVPlayer', 'icons')
    ...
    def stepCopyOnlyIcons(self):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): stepCopyOnlyIcons(self):")
    ...
    def stepCopyPostInatalledBinaries(self, init=True, code=0, msg=''):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): stepCopyPostInatalledBinaries(self, init=True, code=0, msg=''):")
        # get users scripts
        if init:
    ...
    def stepExecuteUserScripts(self, init=True, code=0, msg=''):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): stepExecuteUserScripts(self, init=True, code=0, msg=''):")
        # get users scripts
        if init:
    ...
    def stepConfirmInstalation(self, confirmed=None):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): stepConfirmInstalation(self, confirmed=None):")
        if None == confirmed:
            self.session.openWithCallback(self.stepConfirmInstalation, MessageBox, _("Version [%s] is ready for installation.\nSystem will restart when installation is complete.\n\nDo you want to continue?") % self.serversList[self.currServIdx]['version'], type = MessageBox.TYPE_YESNO, timeout = -1)
        else:
            if confirmed:
    ...
    def stepRemoveOldVersion(self):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): stepRemoveOldVersion(self):")
        cmd = 'rm -rf "%s"/*' % ( os_path.join(self.ExtensionPath, 'IPTVPlayer') )
    ...
    def stepInstallNewVersion(self):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): stepInstallNewVersion(self):")
        cmd = ''
        try: 
            url = "https://www.softrix.co.uk/istream/reporting/check.php?ver=%s&type=%s" % (self.serversList[self.currServIdx]['version'], self.serversList[self.currServIdx]['pyver'])
            cmd = '%s "%s" -t 1 -T 10 -O - > /dev/null 2>&1; ' % (config.plugins.iptvplayer.wgetpath.value, url)
            printDBG("..:: E2iStream ::..   stepInstallNewVersion(self): cmd = [%s], url = [%s]" % (cmd, url))
        except Exception: 
            printExc()
        
        cmd += 'cp -rf "%s"/* "%s"/ 2>&1' % (os_path.join(self.ExtensionTmpPath, 'IPTVPlayer'), os_path.join(self.ExtensionPath, 'IPTVPlayer'))
        printDBG("..:: E2iStream ::..   stepInstallNewVersion(self): cmd = [%s]" % cmd)
        self.cmd = iptv_system( cmd, self.__installNewVersionCmdFinished )

    def downloadFinished(self, callBackFun, arg, status):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): downloadFinished file[%s], status[%s]" % (file, status))
    ...
    def __serversListDownloadFinished(self, arg, status):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): __serversListDownloadFinished(self, arg, status):")
        ...
        try:
            ...
            printDBG('Version of the current installation [%s]' % GetIPTVPlayerVerstion())
            currVerNum = 0
    ...
    def __selServerCallBack(self, retArg):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): __selServerCallBack(self, retArg):")
    ...
    def __archiveDownloadFinished(self, arg, status):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): __archiveDownloadFinished(self, arg, status):")
        url            = self.downloader.getUrl()
        filePath       = self.downloader.getFullFileName()
    ...
    def __encKeyDownloadFinished(self, arg, status):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): __encKeyDownloadFinished(self, arg, status):")
        url            = self.downloader.getUrl()
        filePath       = self.downloader.getFullFileName()
        remoteFileSize = self.downloader.getRemoteFileSize()
        localFileSize  = self.downloader.getLocalFileSize(True)
        ...
        if DMHelper.STS.DOWNLOADED != status and remoteFileSize > localFileSize:
            ...
            self.stepFinished(-1, _("Wrong encryption key size: %s\n") % localFileSize)
        else:
            try:
                ...
                if check != self.serversList[self.currServIdx]['enc']:
                    self.stepFinished(-1, _("Problem with decrypting the key."))
                    return
                self.decKey = data
            except Exception:
                printExc()
    ...
    def __unpackCmdFinished(self, status, outData):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): __unpackCmdFinished(self, status, outData):")
        code = -1
        msg  = ""
        self.cmd = None
        if 0 != status:
    ...
    def __decryptionCmdFinished(self, status, outData):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): __decryptionCmdFinished(self, status, outData):")
        ...
        if 0 != status:
            code = -1
            msg  = _("Problem with decryption of the archive. Return code [%d]\n%s.") % (status, outData)
        else:
            try:
                os_remove
            except Exception:
                printExc()
            code = 0
            msg  = _("Decryption of the archive completed successfully.")
        self.stepFinished(code, msg)
    ...
    def __removeUnnecessaryFilesCmdFinished(self, status, outData):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): __removeUnnecessaryFilesCmdFinished(self, status, outData):")
        code = -1
        msg  = ""
        self.cmd = None
        if 0 != status:
    ...
    def __getUserCmdsList(self):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): __getUserCmdsList(self):")
        cmdList = [] 
        try:
    ...
    def __copyBinariesCmdFinished(self, status, outData):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): __copyBinariesCmdFinished(self, status, outData): status = [%s], outdata = [%s]" %(status, outData))
        self.cmd = None
        if 0 != status:
            code = -1
        else:
    ...
    def __userCmdFinished(self, status, outData):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): __userCmdFinished(self, status, outData): status = [%s], outdata = [%s]" %(status, outData))
        self.cmd = None
        if 0 != status:
            code = -1
        else:
    ...
    def __copyOldCmdFinished(self, status, outData):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): __copyOldCmdFinished(self, status, outData): status = [%s], outdata = [%s]" %(status, outData))
        self.cmd = None
        if 0 != status:
            code = -1
    ...
    def __removeOldVersionCmdFinished(self, status, outData):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): __removeOldVersionCmdFinished(self, status, outData): status = [%s], outdata = [%s]" %(status, outData))
        self.cmd = None
        if 0 != status:
            code = -1
    ...
    def __installNewVersionCmdFinished(self, status, outData):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): __installNewVersionCmdFinished(self, status, outData): status = [%s], outdata = [%s]" %(status, outData))
        self.cmd = None
        if 0 != status:
    ...
    def __doSyncCallBack(self, status, outData):
        printDBG("..:: E2iStream ::..   UpdateMainAppImpl(IUpdateObjectInterface): __doSyncCallBack(self, status, outData): status = [%s], outdata = [%s]" %(status, outData))
        self.cmd = None
