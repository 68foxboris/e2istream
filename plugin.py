...
def Plugins(**kwargs):
    ...
    desc = _("iStream Videos Online")
    list = []
    if config.plugins.iptvplayer.plugin_autostart.value:
        if config.plugins.iptvplayer.plugin_autostart_method.value == 'wizard':
            list.append(PluginDescriptor(name=(("E2iStream")), description=desc, where = [PluginDescriptor.WHERE_WIZARD], fnc=(9, pluginAutostart), needsRestart=False))
    ...
    list.append(PluginDescriptor(name=(("E2iStream")), description=desc, where = [PluginDescriptor.WHERE_PLUGINMENU], icon=iconFile, fnc=main)) # always show in plugin menu
    list.append(PluginDescriptor(name=(("E2iStream")), description=desc, where = PluginDescriptor.WHERE_MENU, fnc=startIPTVfromMenu))
    if config.plugins.iptvplayer.showinextensions.value:
        list.append (PluginDescriptor(name=(("E2iStream")), description=desc, where = [PluginDescriptor.WHERE_EXTENSIONSMENU], fnc=main))
...
def startIPTVfromMenu(menuid, **kwargs):
    if menuid == "system":
        return [(_("Configure %s") % 'E2iStream', mainSetup, "iptv_config", None)]
    elif menuid == "mainmenu" and config.plugins.iptvplayer.showinMainMenu.value == True:
        return [("E2iStream", main, "iptv_main", None)]
    else:
        return []
